...
Using Mahmood Lab's UNI2-h foundation model for pathology (ViT-H/14, 681M params)
"""
...
sys.path.append('/mnt/f/Projects/HoneyBee/honeybee/models/UNI2')
...
class WSIProcessor:
    ...
    def process_slide(self, slide_path, max_patches=200, apply_normalization=False):
        ...
        embeddings_list = []
        embedding_batch_size = 16  # Reduced batch size for ViT-g model
...
def create_tsne_visualization(X_norm, X_no_norm, y, labels, save_dir, perplexity=30, n_iter=1000):
    ...
    plt.suptitle('t-SNE Visualization of UNI2-h Embeddings: Impact of Stain Normalization', fontsize=18)
    plt.tight_layout()
...
def create_additional_visualizations(X_norm, X_no_norm, y, labels, results_norm, results_no_norm, save_dir):
    ...
    plt.suptitle('PCA Visualization of UNI2-h Embeddings', fontsize=16)
    plt.tight_layout()
...
def main():
    ...
    # Configuration
    config = {
        'model_path': None,  # UNI2-h loads from HuggingFace
        'tissue_detector_path': '/mnt/d/Models/TissueDetector/HnE.pt',
        'output_dir': '/mnt/f/Projects/HoneyBee/results/staining_uni2',
        'temp_dir': '/mnt/f/Projects/HoneyBee/results/staining_uni2/tmp',
        'max_patches_per_slide': 100,  # Reduced for UNI2-h (ViT-g model)
        'pooling_method': 'mean_max',  # mean, max, or mean_max
        'normalization_method': 'macenko',  # macenko or reinhard
        'max_slides_per_type': 100  # Start with very few slides for UNI2-h testing  
    }
    ...
    with open(os.path.join(config['output_dir'], 'summary_report.txt'), 'w') as f:
        f.write("Stain Normalization Impact on UNI2-h Embeddings\n")
        f.write("=" * 60 + "\n\n")
