...
sys.path.append('/mnt/f/Projects/HoneyBee/honeybee/models/UNI')
...
class WSIProcessor:
    ...
    def __init__(self, uni_model_path, tissue_detector_path, device='cuda'):
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')
        print(f"WSIProcessor using device: {self.device}")
        self.uni_model = UNI(uni_model_path)
    ...
    def process_slide(self, slide_path, max_patches=200, apply_normalization=False):
        ...
        for i in range(0, len(patches), embedding_batch_size):
            ...
            batch_embeddings = self.uni_model.load_model_and_predict(batch)
...
def create_tsne_visualization(X_norm, X_no_norm, y, labels, save_dir, perplexity=30, n_iter=1000):
    ...
    plt.suptitle('t-SNE Visualization of UNI Embeddings: Impact of Stain Normalization', fontsize=18)
    plt.tight_layout()
...
def create_additional_visualizations(X_norm, X_no_norm, y, labels, results_norm, results_no_norm, save_dir):
    ...
    plt.suptitle('PCA Visualization of UNI Embeddings', fontsize=16)
    plt.tight_layout()
...
def main():
    ...
    config = {
        ...
        'output_dir': '/mnt/f/Projects/HoneyBee/results/staining',
        'temp_dir': '/mnt/f/Projects/HoneyBee/results/staining/tmp',
        'max_patches_per_slide': 100,
        'pooling_method': 'mean_max',  # mean, max, or mean_max
        'normalization_method': 'macenko',  # macenko or reinhard
        'max_slides_per_type': 100  
    }
    ...
    with open(os.path.join(config['output_dir'], 'summary_report.txt'), 'w') as f:
        f.write("Stain Normalization Impact on UNIv2 Embeddings\n")
        f.write("=" * 60 + "\n\n")
