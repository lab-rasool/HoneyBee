...
class RadImageNet:
    ...
    def _load_model(self) -> None:
        ...
        if self.model_filename:
            ...
            if "DenseNet121" in model_filename:
                ...
                raise ValueError(f"Cannot determine model type from filename: {model_filename}")
        else:
        ...
        if self.use_hub:
            ...
            try:
                ...
                self.model_path = hf_hub_download(repo_id=self.repo_id, filename=model_filename)
    ...
    def _process_3d(self, input_tensor: torch.Tensor, aggregation: str = "mean") -> torch.Tensor:
        ...
        if len(input_tensor.shape) != 4:
            raise ValueError(
                f"Expected 4D tensor for 3D processing, got {len(input_tensor.shape)}D"
            )
    ...
    def process_batch(
        self, images: List[Union[np.ndarray, torch.Tensor]], batch_size: int = 32
    ) -> torch.Tensor:
    ...
    def extract_multi_scale_features(
        self, input_data: Union[torch.Tensor, np.ndarray], scales: List[float] = [0.5, 1.0, 1.5]
