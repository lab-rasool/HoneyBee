...
class RadImageNet:
    ...
    def _load_model(self) -> None:
        if 'DenseNet121' in self.model_path:
            ...
            raise ValueError("Model not recognized. Ensure the model path contains one of 'DenseNet121', 'InceptionV3', or 'ResNet50'.")
    ...
    def load_model_and_predict(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        if len(input_tensor.shape) != 4:
            raise ValueError("Input tensor must be of shape [slices, 3, 224 or 299, 224 or 299].")

        # Handle input size for InceptionV3
        if isinstance(self.model, models.Inception3):
            if input_tensor.shape[2:] != (299, 299):
                raise ValueError("InceptionV3 model requires input size of [slices, 3, 299, 299].")
        else:
            if input_tensor.shape[2:] != (224, 224):
                raise ValueError("DenseNet121 and ResNet50 models require input size of [slices, 3, 224, 224].")
