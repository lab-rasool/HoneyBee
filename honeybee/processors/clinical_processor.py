...
BIOMEDICAL_MODELS = {
    ...
    "gatortron": {
        ...
        "gated": True,  # Requires access request
    },
    "clinicalt5": {
...
ONTOLOGY_MAPPINGS = {
    ...
    "loinc": {
        "hemoglobin": {"id": "718-7", "name": "Hemoglobin [Mass/volume] in Blood"},
        "white blood cell count": {"id": "6690-2", "name": "Leukocytes [#/volume] in Blood"},
        "platelet count": {"id": "777-3", "name": "Platelets [#/volume] in Blood"},
        "creatinine": {"id": "2160-0", "name": "Creatinine [Mass/volume] in Serum or Plasma"},
        "glucose": {"id": "2345-7", "name": "Glucose [Mass/volume] in Serum or Plasma"},
        "albumin": {"id": "1751-7", "name": "Albumin [Mass/volume] in Serum or Plasma"},
        "bilirubin": {"id": "1975-2", "name": "Bilirubin.total [Mass/volume] in Serum or Plasma"},
        "calcium": {"id": "17861-6", "name": "Calcium [Mass/volume] in Serum or Plasma"},
        "potassium": {"id": "2823-3", "name": "Potassium [Moles/volume] in Serum or Plasma"},
        "sodium": {"id": "2951-2", "name": "Sodium [Moles/volume] in Serum or Plasma"},
    },
}
...
class ClinicalProcessor:
    ...
    def _init_components(self):
        ...
        # Warn about documented-but-not-implemented config options
        er_config = self.config.get("entity_recognition", {})
        if er_config.get("use_spacy", False):
            warnings.warn(
                "use_spacy is not yet implemented. spaCy-based NER will be skipped. "
                "Rule-based and pattern-based extraction will still run.",
                stacklevel=2,
            )
        if er_config.get("use_deep_learning", False):
            warnings.warn(
                "use_deep_learning is not yet implemented. Deep learning NER will be skipped. "
                "Rule-based and pattern-based extraction will still run.",
                stacklevel=2,
            )
        if er_config.get("term_disambiguation", False):
            warnings.warn(
                "term_disambiguation is not yet implemented and will be ignored.",
                stacklevel=2,
            )

        ontologies = er_config.get("ontologies", [])
        unsupported = [o for o in ontologies if o not in ONTOLOGY_MAPPINGS]
        if unsupported:
            warnings.warn(
                f"Ontologies not yet supported and will be skipped: {unsupported}. "
        ...
        try:
            ...
            if "document" in self.config["processing_pipeline"]:
                ...
                result["document_processing"] = {
                    ...
                    "confidence": document_result.get(
                        "avg_confidence", document_result.get("confidence", 0)
                    ),
                }
            ...
            if (
                ...
                tokenization_result = self._tokenize_text(
                    result["text"], result.get("document_structure")
                )
            ...
            if "entity_recognition" in self.config["processing_pipeline"] and result.get("text"):
                ...
                if (
                    self.config.get("output", {}).get("include_temporal_timeline", True)
            ...
            # NOTE: Automatic embedding generation during process() is disabled
            # to avoid loading models unnecessarily and consuming memory.
            # Use the public generate_embeddings() method explicitly if needed:
    ...
    def process_text(self, text: str, document_type: str = "unknown") -> Dict:
        ...
        try:
            ...
            # NOTE: Automatic embedding generation during process_text() is disabled
            # to avoid loading models unnecessarily. Use generate_embeddings() explicitly:
            ...
            if self.config.get("document_processor", {}).get("use_ocr", True):
                try:
                    ...
                    result["avg_confidence"] = (
                        np.mean(result["confidence_scores"]) if result["confidence_scores"] else 0
                    )
                except Exception as e:
    ...
    def _process_image(self, file_path: Path) -> Dict:
        ...
        result = {"file_path": str(file_path), "file_type": "image", "text": "", "confidence": 0}
    ...
    def _post_process_ocr_text(self, text: str) -> str:
        ...
        # Expand abbreviations if enabled (using word boundaries for correct matching)
        # NOTE: _clean_text also expands abbreviations. For OCR text that goes through
        # both paths, the second expansion is a no-op since abbreviations are already replaced.
    ...
    def _process_xml_ehr(self, file_path: Path) -> Dict:
        ...
        return {"text": "\n".join(text_parts), "extraction_method": "xml_parse"}
    ...
    def _clean_text(self, text: str) -> str:
        ...
        # Expand abbreviations using the shared MEDICAL_ABBREVIATIONS dict
        # Uses word-boundary regex to handle start/end of text and punctuation
        if self.config.get("entity_recognition", {}).get("abbreviation_expansion", True):
            for abbr, expansion in MEDICAL_ABBREVIATIONS.items():
                text = re.sub(rf"\b{re.escape(abbr)}\b", expansion, text, flags=re.IGNORECASE)
    ...
    def _handle_long_document(self, segments: List[str]) -> Dict:
        ...
        if strategy == "sliding_window":
            ...
            warnings.warn(
                "long_document_strategy='summarize' is not yet implemented. "
                "Falling back to default tokenization (truncation).",
                stacklevel=2,
            )
            return self._tokenize_segments(segments)
        else:
    ...
    def _sliding_window_tokenization(self, segments: List[str]) -> Dict:
        ...
        full_encoding = self.tokenizer(
            full_text, add_special_tokens=False, truncation=False, return_offsets_mapping=True
        )

        all_input_ids = full_encoding["input_ids"]
        stride = self.config.get("tokenization", {}).get("stride", 128)

        # Determine special tokens for this model.
        # Models like T5/SciFive don't have CLS/SEP tokens — use EOS as suffix instead.
        bos_id = self.tokenizer.cls_token_id  # None for T5-family
        eos_id = self.tokenizer.sep_token_id  # None for T5-family
        if eos_id is None:
            eos_id = self.tokenizer.eos_token_id  # T5 uses eos_token_id
        ...
        if pad_id is None:
            pad_id = 0  # fallback for models without an explicit pad token
        ...
        for i in range(0, len(all_input_ids), max_length - stride):
            ...
            # Build window with available special tokens
            window_ids = prefix_ids + all_input_ids[i:end_idx] + suffix_ids
    ...
    def _hierarchical_tokenization(self, segments: List[str]) -> Dict:
        ...
        results = {"sections": {}, "tokenization_strategy": "hierarchical"}
    ...
    def _extract_rule_based_entities(self, text: str) -> List[Dict]:
        ...
        # Medication patterns
        med_pattern = (
            r"(?i)(\w+)\s+(\d+)\s*(mg|mcg|g|ml|units?)\s*(daily|bid|tid|qid|prn|po|iv|im|sq)"
        )
        ...
        # Lab values — require key to look like a lab test name (1-4 words, no digits)
        # and unit to be a known clinical unit to avoid matching headings/metadata
        lab_pattern = r"(?<!\w)([A-Za-z][A-Za-z\s]{0,30}?):\s*(\d+\.?\d*)\s*(mg/dL|g/dL|mL|mg|mcg|mmol/L|mEq/L|U/L|IU/L|ng/mL|pg/mL|µg/dL|%|cells/µL|mm3|x10[³⁹]?/[µu]?L|mmHg|bpm|K/uL|M/uL|fL|pg)"
    ...
    def _extract_pattern_based_entities(self, text: str) -> List[Dict]:
        ...
        # Condition patterns — also match at end of text/line via $
            ...
            entities.append(
                {
                    ...
                    "start": match.start(2),
                    "end": match.end(2),
                    "properties": {"context": match.group(1), "source": "pattern-based"},
                }
            )
        ...
        for pattern_name, pattern in self.compiled_patterns.items():
            for match in pattern.finditer(text):
                ...
                entity = {
                    ...
                    "start": match.start(),
                    "end": match.end(),
                    "properties": {"pattern": pattern_name, "source": "cancer-specific"},
                }
    ...
    def _merge_entities(self, entities: List[Dict]) -> List[Dict]:
        """Merge overlapping entities, preserving entities of different types"""
        if not entities:
            return []
        ...
        for entity in entities[1:]:
            ...
            if entity["start"] < current["end"]:
                if entity["type"] != current["type"]:
                    # Different types — keep both (clinically distinct information)
                    merged.append(current)
                    current = entity
                else:
                    # Same type overlap — keep longer or more specific
                    if entity["end"] > current["end"] or self._is_more_specific(
        ...
        for i, entity1 in enumerate(entities):
            for j, entity2 in enumerate(entities[i + 1 :], i + 1):
                ...
                if entity2["start"] - entity1["end"] < 50:
                    ...
                    if rel_type:
                        relationships.append({"source": i, "target": j, "type": rel_type})
    ...
    # NOTE: The _generate_embeddings() method has been removed as it was dead code.
    # self.embedder was never initialized (commented out in _init_components).
    # Use the public generate_embeddings() method instead, which properly
    # initializes HuggingFaceEmbedder with any model.
    ...
    def generate_embeddings(
        ...
        """
        Generate embeddings for clinical text using any HuggingFace language model
        ...
        Args:
            ...
            model_name: Model to use. Can be:
                       - Preset name: "bioclinicalbert", "pubmedbert", "biobert",
                         "scibert", "clinicalt5", "gatortron" (requires access),
                         "sentence-transformers"
                       - Any HuggingFace model ID: "bert-base-uncased",
                         "dmis-lab/biobert-v1.1", etc.
                       - If None, uses model from config or defaults to "bioclinicalbert"
            pooling_method: How to pool token embeddings. Options:
                          - "mean": Mean pooling (default, recommended)
                          - "cls": Use CLS token embedding
                          - "max": Max pooling
                          - "pooler_output": Use model's pooler output (if available)
        ...
        Raises:
            ModelAccessError: If model requires authentication/access approval
            ModelNotFoundError: If model doesn't exist on HuggingFace Hub
            RuntimeError: For other model loading errors
        ...
        Examples:
            ...
            >>> embeddings = processor.generate_embeddings(text, model_name="bioclinicalbert")
            ...
            >>> embeddings = processor.generate_embeddings(texts, batch_size=8)
        """
        # Import HuggingFaceEmbedder and exceptions
        try:
            from ..models import HuggingFaceEmbedder, ModelAccessError, ModelNotFoundError
        except ImportError:
            raise ImportError("HuggingFaceEmbedder not available. Please check installation.")

        # Determine which model to use
        if model_name is None:
            model_name = self.config.get("tokenization", {}).get("model", "bioclinicalbert")

        # Check if model_name is a preset or a direct HuggingFace model ID
        if model_name in BIOMEDICAL_MODELS:
            # It's a preset - get the configuration
            ...
            # Warn if model is gated
            if model_config.get("gated", False):
                self.logger.warning(
                    f"Model '{model_name}' ({hf_model_id}) requires access approval. "
                    f"If you haven't been granted access, consider using an open model like "
                    f"'bioclinicalbert' or 'pubmedbert' instead."
                )

            self.logger.info(
                f"Using preset '{model_name}': {model_config.get('description', 'No description')}"
            )
        else:
            # Treat as direct HuggingFace model ID
            hf_model_id = model_name
            default_max_length = 512
            self.logger.info(f"Using HuggingFace model: {hf_model_id}")

        # Use provided max_length or default
        final_max_length = max_length if max_length is not None else default_max_length
        ...
        try:
            ...
            # Re-raise these with full context - they already have helpful messages
            raise
        except Exception as e:
            # Wrap unexpected errors with helpful context
            self.logger.error(f"Failed to load model '{hf_model_id}': {e}")

            # Get list of recommended open models
            recommended = HuggingFaceEmbedder.get_recommended_open_models()
            suggestion_msg = "\n\nTry these open-access biomedical models:\n"
            for model in recommended["biomedical"][:3]:  # Show top 3
                suggestion_msg += f"  • {model['name']}\n"

            raise RuntimeError(
                f"Failed to load model '{hf_model_id}': {type(e).__name__}: {e}"
                + suggestion_msg
            ) from e

        # Generate embeddings
        try:
            self.logger.info(f"Generating embeddings for {len(text) if isinstance(text, list) else 1} text(s)")
            ...
            raise RuntimeError(
                ...
                f"Model: {hf_model_id}, Pooling: {pooling_method}, Batch size: {batch_size}"
            ) from e
