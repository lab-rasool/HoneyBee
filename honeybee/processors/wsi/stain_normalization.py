...
class MacenkoNormalizer:
    ...
    def transform(
        self, source_image: np.ndarray, tissue_mask: Optional[np.ndarray] = None
    ) -> np.ndarray:
        """Transform source image using fitted parameters."""
        ...
        if "concentration_stats" in params:
            ...
            self.target_concentrations_stats = {
                ...
                "percentiles": np.array(
                    [[0.2, 0.05], [0.5, 0.2], [0.7, 0.3], [0.9, 0.4], [1.2, 0.6]]
                ),
            }
        return self
        ...
        for i in range(2):  # For H and E
            if source_std[i] > 0:
                ...
                normalized_concentrations[:, i] = (
                    source_concentrations[:, i] - source_mean[i]
                ) / source_std[i]
                normalized_concentrations[:, i] = (
                    normalized_concentrations[:, i] * self.target_concentrations_stats["std"][i]
                    + self.target_concentrations_stats["mean"][i]
                )
                # Ensure non-negative concentrations
                normalized_concentrations[:, i] = np.maximum(normalized_concentrations[:, i], 0)
                # Clip extreme values
                normalized_concentrations[:, i] = np.minimum(
                    normalized_concentrations[:, i],
                    self.target_concentrations_stats["mean"][i]
                    + 3 * self.target_concentrations_stats["std"][i],
                )
...
def normalize_macenko(
    source: np.ndarray, target: np.ndarray, use_tissue_mask: bool = True
) -> np.ndarray:
...
def normalize_stain_tissue_aware(
    source: np.ndarray,
    target: np.ndarray,
    method: str = "macenko",
...
class ColorAugmenter:
    ...
    def augment(self, image: np.ndarray, seed: Optional[int] = None) -> np.ndarray:
        ...
        if self.brightness_range != (1, 1):
            ...
            augmented = np.clip(augmented.astype(np.float32) * brightness_scale, 0, 255).astype(
                np.uint8
            )
        ...
        if self.contrast_range != (1, 1):
            ...
            mean = augmented.mean()
            augmented = np.clip(
                (augmented.astype(np.float32) - mean) * contrast_scale + mean, 0, 255
            ).astype(np.uint8)
