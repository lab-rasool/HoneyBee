...
def normalize_macenko_fixed(source: np.ndarray, target: np.ndarray) -> np.ndarray:
    ...
    def get_stain_matrix(image):
        ...
        if len(od_flat) < 100:
            ...
            return np.array([[0.650, 0.072], [0.704, 0.990], [0.286, 0.105]])
    ...
    source_concentrations = np.linalg.lstsq(
        stain_matrix_source, od_source.reshape(-1, 3).T, rcond=None
    )[0].T
    ...
    target_concentrations = np.linalg.lstsq(
        stain_matrix_target, od_target.reshape(-1, 3).T, rcond=None
    )[0].T
...
def normalize_with_tissue_mask(
    source: np.ndarray, target: np.ndarray, threshold: int = 235
) -> np.ndarray:
    """
