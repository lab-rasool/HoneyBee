...
class StainSeparator:
    ...
    def _separate_custom(
        ...
        d_channel = (
            concentrations[:, :, 2] if concentrations.shape[2] > 2 else np.zeros_like(h_channel)
        )
        ...
        d_rgb = (
            self._reconstruct_single_stain(d_channel, stain_matrix[:, 2])
    ...
    def _create_single_stain_rgb(
        self, channel: np.ndarray, channel_idx: int, shape: Tuple
    ) -> np.ndarray:
    ...
    def _reconstruct_single_stain(
        self, concentration: np.ndarray, stain_vector: np.ndarray
    ) -> np.ndarray:
    ...
    def enhance_stain(
        self, image: np.ndarray, h_factor: float = 1.0, e_factor: float = 1.0, d_factor: float = 1.0
    ) -> np.ndarray:
        """
