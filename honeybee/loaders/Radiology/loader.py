...
class DicomLoader:
    ...
    def load_series(
        self, directory: Union[str, Path], series_uid: Optional[str] = None
    ...
    def _extract_metadata(self, ds: pydicom.Dataset) -> ImageMetadata:
        ...
        scanner_model = (
            str(ds.ManufacturerModelName) if hasattr(ds, "ManufacturerModelName") else None
        )
...
class NiftiLoader:
    ...
    def save_file(self, data: np.ndarray, metadata: ImageMetadata, filepath: Union[str, Path]):
...
def load_dicom_series(
    directory: Union[str, Path], series_uid: Optional[str] = None
) -> Tuple[np.ndarray, ImageMetadata]:
